        ...
        begin
          ...
          begin
            ...
            if DefaultDrawing and not (csDesigning in ComponentState) and
               not (TStyleManager.IsCustomStyleActive and (goDrawFocusSelected in FOptions)) and
...
begin
  ...
  begin
    ...
    if TStyleManager.IsCustomStyleActive and UseRightToLeftAlignment then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not (seClient in StyleElements) then AColor := Self.Color;
      end;
...
begin
  ...
  begin
    ...
    Flags := 0;
    LStyle := StyleServices;
    if not (seClient in StyleElements) or not TStyleManager.IsCustomStyleActive then
    begin
      UpdateR := ClientRect;
    ...
    case EditStyle of
      ...
      esEllipsis:
        begin
          ...
          W := ButtonWidth shr 3;
          if W = 0 then W := 1;
          if LStyle.Enabled and TStyleManager.IsCustomStyleActive then
          begin
            FCanvas := TCanvas.Create;
            try
              FCanvas.Handle := DC;
...
begin
  inherited;
  if StyleServices.Enabled and not FMouseInControl then
  begin
